...
// ─── Step 1: DNS Resolution + Reverse DNS + WHOIS ───

func dnsRecon(ctx context.Context, sessionID, host string) DNSResult {
	publishEvent(ctx, sessionID, "recon", "dns", fmt.Sprintf("DNS resolution for %s...", host), nil)

	result := DNSResult{ResolvedIP: "N/A", ReverseDNS: "N/A"}
	...
	if err == nil && len(ips) > 0 {
		...
		// Reverse DNS
		names, err := net.LookupAddr(result.ResolvedIP)
		if err == nil && len(names) > 0 {
			result.ReverseDNS = strings.TrimSuffix(names[0], ".")
		}
	}

	// Simple WHOIS lookup via TCP
	result.WHOIS = simpleWHOIS(host)

	publishEvent(ctx, sessionID, "recon", "dns",
		fmt.Sprintf("DNS: %s → %s (reverse: %s)", host, result.ResolvedIP, result.ReverseDNS),
		map[string]interface{}{
			"resolved_ip": result.ResolvedIP,
			"reverse_dns": result.ReverseDNS,
			"whois_org":   result.WHOIS.Org,
		})
	return result
}

func simpleWHOIS(domain string) WHOISResult {
	w := WHOISResult{Org: "N/A", Registrar: "N/A"}

	// Extract root domain (basic: last two parts)
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "refer:") {
			referServer = strings.TrimSpace(strings.TrimPrefix(line, "refer:"))
		}
	}
	...
	// Query the referred WHOIS server
	conn2, err := net.DialTimeout("tcp", referServer+":43", 5*time.Second)
	if err != nil {
		return w
	}
	defer conn2.Close()
	conn2.SetDeadline(time.Now().Add(8 * time.Second))
	...
	for scanner2.Scan() {
		...
		if strings.HasPrefix(lower, "registrant organization:") || strings.HasPrefix(lower, "org-name:") {
			w.Org = strings.TrimSpace(line[strings.Index(line, ":")+1:])
		}
		if strings.HasPrefix(lower, "registrar:") {
			w.Registrar = strings.TrimSpace(line[strings.Index(line, ":")+1:])
		}
	}
	return w
}

// ─── Step 2: TCP Port Scan — Top 1000 Ports ───

func scanPorts(ctx context.Context, sessionID, host, scope string) []int {
	publishEvent(ctx, sessionID, "recon", "port-scan", fmt.Sprintf("Starting TCP port scan (scope: %s)...", scope), nil)
	...
	commonPorts := []int{
		...
		1113, 1114, 1117, 1119, 1121, 1122, 1131, 1138, 1148, 1152, 1169, 1174,
		...
		10629, 10778, 11110, 11111, 11211, 11434, 11967, 12000, 12174, 12265,
		13456, 13722, 14000, 14238, 14441, 14442, 15000, 15002, 15003, 15004,
	...
	if scope == "web" {
		commonPorts = []int{80, 443, 8080, 8443, 3000, 4000, 5000, 8000, 8081, 9000}
	}
	...
	for _, p := range commonPorts {
		...
		go func(port int) {
			...
			if err == nil {
				...
				mu.Unlock()
				publishEvent(ctx, sessionID, "recon", "port-scan",
					fmt.Sprintf("Port %d OPEN", port),
					map[string]interface{}{"port": port, "status": "open"})
			}
		}(p)
	}
	wg.Wait()
	...
	publishEvent(ctx, sessionID, "recon", "port-scan",
		fmt.Sprintf("Port scan complete — %d open ports found", len(openPorts)),
		map[string]interface{}{"open_ports": openPorts, "total": len(openPorts)})
